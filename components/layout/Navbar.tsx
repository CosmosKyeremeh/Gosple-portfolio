...
const links = [
  ...
  { href: '/math-vector', label: 'Vectors',    icon: Sigma },
...
export default function Navbar() {
  ...
  return (
    <>
      {/* 1. Profile Avatar Container (Absolute on mobile to scroll away · Fixed on desktop to stay locked) */}
      <div className="absolute md:fixed top-6 left-0 right-0 z-40 px-4 pointer-events-none">
            ...
            {links.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={href}
                  href={href}
                  className={`flex items-center gap-2 rounded-full font-mono text-xs transition-all duration-200 flex-shrink-0 ${
                    isActive
